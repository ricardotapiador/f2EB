...
namespace ImageDvs
{
    public partial class FRAME2DVS : MaterialForm
    {
        public FRAME2DVS()
        {
            InitializeComponent();
        ...
        private void Scan_conversion(float[,] pixel_matrix, BinaryWriter bWriter, Bitmap bitmap, bool oneframe)
        {
            UInt32 evt;
            UInt32 timestamp;
            ...
            for (int i = 0; i < 255; i++)
            {
                for (int y = 0; y < bitmap.Height; y++)
                {
                    for (int x = 0; x < bitmap.Width; x++)
                    {
                        ...
                        if (Math.Round(pixel_matrix[x, y] * 256) > i)
                        {
                            addr = (UInt32)((127 - y) << 8 | (127 - x) << 1 | 1);
        ...
        private void Random_Method(float[,] pixel_matrix, BinaryWriter bWriter, bool oneframe)
        {
            ...
            for (int i = 0; i < Math.Pow(2, 16); i++)
            {
                ...
                UInt32 max = (UInt32)Math.Pow(2, 30);
                int LFSR = rnd.Next(1, (int)max);
        ...
        private void Bitwise_method(float[,] pixel_matrix, BinaryWriter bWriter, bool oneframe)
        {
            ...
            for (Int32 i = 0; i < Math.Pow(2, 18); i++)
            {

                Int32 n = i;
                string binary_reverse = Reverse(Convert.ToString(n, 2).PadLeft(22, '0'));
                UInt32 reversed_count = Convert.ToUInt32(binary_reverse, 2);
                Console.WriteLine(reversed_count);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (saveFileDialog1.FileName != ""&&convert_method_list.Text!="")
                {
                    ...
                    {
                        Scan_conversion(pixel_matrix, bWriter, current_image, true);
                    }
                    ...
                    if (method != "")
                    {
                        new Thread(() =>
                        {
                            ...
                            for (int f = 0; f < image_array.Count; f++)
                            {
                                ...
                                {
                                    case "SCAN":
                                        Scan_conversion(pixel_matrix_frame, bWriter, current_image_frame, oneframe);
                                        break;
                                    case "Random":
